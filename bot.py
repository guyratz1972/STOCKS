...
async def scheduled_open(context: ContextTypes.DEFAULT_TYPE):
    logger.info("Running scheduled open report")
...
async def scheduled_close(context: ContextTypes.DEFAULT_TYPE):
    logger.info("Running scheduled close report")
...
def main():
    ...
    # Schedule using UTC times (Israel is UTC+3)
    # 16:30 Israel = 13:30 UTC
    # 22:00 Israel = 19:00 UTC
    jq.run_daily(scheduled_open,  time=time(13, 30), days=(0, 1, 2, 3, 4))
    jq.run_daily(scheduled_close, time=time(19, 0),  days=(0, 1, 2, 3, 4))

    logger.info("Bot started. Scheduled: 13:30 UTC (16:30 IL) and 19:00 UTC (22:00 IL)")
    app.run_polling()
